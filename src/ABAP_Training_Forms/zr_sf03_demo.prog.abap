...
*&---------------------------------------------------------------*
*& SF03 - Smartform 呼叫端骨架示範（Logo/Graphic）
*& 查出 ZSF_03_LOGO 動態產生的 Function Module 名稱，
...
WRITE: / 'SF03 - Smartform 呼叫端示範（Logo/Graphic）', /.

CALL FUNCTION 'SSF_FUNCTION_MODULE_NAME'
  EXPORTING
    formname           = 'ZSF_03_LOGO'
  IMPORTING
    fm_name             = lv_fm_name
  EXCEPTIONS
    no_form             = 1
...
IF sy-subrc <> 0.
  WRITE: / '找不到表單 ZSF_03_LOGO 或尚未啟用，請先在 SMARTFORMS 建立並啟用。'.
  RETURN.
ENDIF.
...
ELSE.
  WRITE: / '呼叫成功，請確認預覽畫面或 Spool 是否印出 Logo 與文字並排版面。'.
ENDIF.
