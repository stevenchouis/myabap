...
- **尚待驗證**：`NUMBER_SWITCH` 的回填邏輯已用單元測試（模擬呼叫，不經真實 BAdI 派送）證實程式邏輯本身正確，但**尚未用真實 `CO01` 建單確認 SAP 在真實存檔流程裡真的會呼叫到這個方法**（單元測試只證明「如果 NUMBER_SWITCH 被呼叫，回填邏輯是對的」，不能證明「SAP 真的會呼叫它」）。下次使用者用 `CO01`（Plant `1011`／Order Type `PP71`）建立真實工單存檔後，應到 SE16 查 `ZEN06_ATSAVE_LOG`：預期只看到**一筆**記錄，且 `AUFNR` 欄位是真正的最終工單號碼（不是 `%00000000001` 這種暫時格式）——這樣才算完整驗證通過。
