...
| en02 | Classic User-Exit 實戰：批號自動給號＋Number Range Object | 真實案例（非虛構）：SAP 標準批號自動給號（`VB_NEXT_BATCH_NUMBER`）同一流程疊了三代擴充技術（Classic User-Exit `EXIT_SAPLV01Z_001`/`_002`、新式 BAdI `BADI_BATCH_NUMBER_INT`、S/4HANA Cloud BAdI），本題實作 `EXIT_SAPLV01Z_002`＋自建 Number Range Object `ZEN02BAT`，把批號改成 `YYMMDD`+4碼流水號（`MCHA-CHARG` 只有 CHAR10，故不用完整日期+時間）；順帶證實「ZX 開頭 Include 保留給 Exit Function Group、無法用一般 ADT Include API 建立」，以及「Include 原始碼寫好啟用≠Enhancement 生效，一定要走 CMOD 建 Project＋Assign＋Activate」 | 承 en01 | **程式碼已完成、CMOD 啟用待使用者處理**（`en02_classic_user_exit_batch_number.md`；`ZEN02BAT`／`ZXVBZU02`〔套件 `ZPP`，傳輸 `S4HK901982`〕／`ZR_EN02_BATCH_DEMO`〔`$TMP`〕均已建立啟用，`programrun` 實測 `ZEN02BAT`＋格式化邏輯本身正確、輸出真實批號 `2607280001`/`0002`/`0003`；`EXIT_SAPLV01Z_002` 這個 Enhancement 要真的對 MIGO 等真實貨物移動生效，還需使用者在 `CMOD` 建 Enhancement Project、Assign `SAPLV01Z`、Activate Project，這步是 GUI-only） |
| en03 | Classic BAdI 觀念與尋找 | BAdI 是 User-Exit 的物件導向後繼者：Definition（`SE18`）／Implementation（`SE19`）分工、Interface 為什麼一定要繼承 `IF_BADI_INTERFACE`、Filter-dependent 用途、`GET BADI`＋`CALL BADI` 呼叫語法；示範查詢一個標準單一實作 BAdI 並讀出其 Interface 定義 | 承 en01/en02，對照 Interface 課程 if01 FM vs BAPI 的「找標準物件」手感 | 待出題 |
