...
- **本課程題號＝授課順序**，rs09（期末整合）是原訂課綱最後一題；rs10 是 2026-07-13 驗收 rs09 後追加的延伸題（使用者要求補上「JSON Array → Internal Table → CALL BAPI」這個原課綱沒有的模式，不算在原本「結業」範圍內，但屬於同一份教材）
...
| rs07 | PUT/DELETE 與統一錯誤格式 | PUT 更新、DELETE 刪除、狀態碼語意（200/204/400/404）、`ZCX_` 例外攔截轉標準 JSON error body，避免未捕捉例外變成 dump | 承 op09 自訂例外類別設計 | 已驗收（GET/PUT 已測；`SPROX_HTTP_REQUEST` 沒有 DELETE 選項，DELETE 待補測） |
...
| rs09 | 期末整合：Flight Booking CRUD REST API | SBOOK 的完整 GET（集合+單筆+篩選）/POST/PUT/DELETE，Service/Resource 分層、統一錯誤格式、ABAP Unit 測試，SICF 掛載後用 Postman/瀏覽器實測全流程 | 對照 op12 期末報表重構的收斂角色 | 待驗收（ABAP Unit 8/8 綠燈；SICF 手動測試待使用者掛載） |
| rs10（延伸題） | JSON Array 批次處理與 BAPI 整合 | POST 收 JSON Array、`/UI2/CL_JSON` 直接反序列化進 Internal Table、`LOOP` 呼叫標準 BAPI `BAPI_FLBOOKING_CREATEFROMDATA` 批次建立訂位、逐筆成功/失敗回報（非全有全無）、`BAPI_TRANSACTION_COMMIT` | 補課綱原本沒有的「Header/Detail、多筆輸入、呼叫 BAPI 更新表格」缺口 | 待驗收（ABAP Unit 2/2 綠燈；SICF 手動測試待使用者掛載） |
