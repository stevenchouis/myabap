...
- **這題的 POST body 刻意直接用陣列當根節點（`[{...}, {...}]`），沒有額外包一層 `{"data": [...]}` 或 `{"REQUEST": [...]}` 這種 wrapper key**——這是刻意的設計選擇，不是隨便省略：HTTP Method（`POST`）本身就已經表達了「這是一個請求」，body 裡再包一層通用命名的外層 key（尤其是像 `REQUEST` 這種完全沒有業務語意的名稱）對前端/呼叫端沒有實質幫助，只是多一層要拆的殼；只有當這個 endpoint 除了這批陣列資料之外，還需要傳遞跟陣列平行的「全域性」參數（例如呼叫者身分 `operator_id`、來源系統代碼）時，才有必要在外層包一個物件、並把陣列放在一個有業務意義的 key 底下（例如 `"bookings": [...]`，而不是通用的 `"data"`/`"REQUEST"`）——這題目前不需要這類全域參數，所以維持陣列直接當根節點
