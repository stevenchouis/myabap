...
- **`header` 這層 key 本身要不要存在，是有爭議的設計選擇，不是唯一解**：這題目前把 `docType`/`salesOrg`/`soldTo` 這些抬頭欄位包在 `header` 物件底下（連 `items` 也巢狀在 `header` 裡），但另一派更「純 REST」的設計主張連 `header` 這層 wrapper 都該拿掉，直接把這些欄位攤平在 JSON 最外層、`items` 變成跟它們同層的一個陣列欄位（`{"docType":"...", "items":[...]}`，不是 `{"header":{"docType":"...","items":[...]}}`）——攤平版本對前端來說少一層要拆的殼（`order.docType` 而不是 `order.header.docType`），语意上也更像「這些欄位本來就是這個資源的屬性」。這題選擇保留 `header` 分組，主要原因是**這樣的結構跟 `BAPISDHD1` 的欄位分組直接對應**，設計/除錯時比較容易對照 BAPI 介面文件；如果之後要新增一題刻意示範「更純的攤平式 REST 設計」，可以拿掉 `header` 這層當作對照組
- **這題只示範「一張訂單（1 個 header）+ 多筆品項」，沒有涵蓋「一次送多張訂單（多個 header，每個各自帶自己的 items）」這種批次情境**——如果要支援後者，JSON body 的根節點會變成一個陣列，陣列裡每個元素才是「一張訂單」（`[{"docType":...,"items":[...]}, {"docType":...,"items":[...]}]`），這跟 rs10「陣列裡每筆是扁平的單一資源」不一樣，是「陣列裡每筆自己還有巢狀結構」——這是這門課目前完全沒示範過的第三種陣列模式（rs03 是輸出扁平陣列、rs10 是輸入扁平陣列、這題是輸入單一巢狀物件），值得當作下一題延伸的候選主題
