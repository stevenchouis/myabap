...
| am06 | AMDP 除錯與資料預覽 | `ZR_AM06_DEMO`：重用 am04 已驗證的 AMDP 方法，改用 **Functional ALV**（`cl_salv_table`）呈現結果；Eclipse ADT 內建 AMDP Debugger 操作步驟（本題唯一需要使用者在 Eclipse 手動操作，Claude 端無法自動驗證）、Data Preview 快速查資料現況 | — | 待使用者確認（ALV 資料邏輯已驗證正確，沿用 am04；AMDP Debugger 操作待使用者於 Eclipse 實際確認） |
