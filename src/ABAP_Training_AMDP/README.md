...
REST 課程（`src/ABAP_Training_REST/`，rs01–rs11）之後的下一階段候選之一。課綱草案（2026-07-19），am01 已出題並驗收（2026-07-19），其餘題目待逐題出題。
...
| am02 | 第一個 AMDP Method：Signature 規則與呼叫方式 | AMDP Method 參數限制（只能用 Table Type，不能是 elementary/structure；只有 IMPORTING/EXPORTING，沒有 RETURNING/CHANGING）；從一般 ABAP 呼叫 AMDP Method 跟呼叫一般 Method 語法上完全一樣（呼叫端無感）；**同時 `EXPORTING` 兩個 Internal Table 的範例**（例如同一次 SQLScript 運算，一次吐出「明細結果」+「彙總統計」兩張表），對照一般 ABAP Method 只能 `RETURNING` 單一值/表格的限制，這是 SQLScript 一次運算多個結果集的重要特性；**呼叫端寫一支 demo 程式，用 `cl_salv_table` 把這兩張回傳的 Internal Table 各自呈現成一個 ALV**，讓學員能實際跑起來看到「一次 AMDP 呼叫、兩個獨立結果集」的效果，不是只停留在程式碼閱讀 | 承 OOP op02 方法參數的對照；ALV 呈現承 OOP op11 `cl_salv_table` | 未出題 |
