...
| am04 | SQLScript 集合處理：多表 JOIN／聚合／CTE | `ZCL_AM04_ROUTE_LOAD`：`WITH ... AS (...)` CTE 先依航線彙總 SFLIGHT，再 JOIN SCARR 補上公司名稱算出每條航線載客率；**實測發現兩個坑**：AMDP 簽章 `USING` 子句多個物件要空白分隔、不是逗號；SQLScript 的 JOIN ON 條件**必須**明寫 MANDT（跟 Open SQL JOIN ON 條件**不能**寫 MANDT 正好相反，兩者都指向「Client 處理自動化在哪一層」這個核心對照）；`get_flight_timeline` 方法補充 `UNION ALL`＋字串函數（`CONCAT`/`UPPER`）＋日期函數（`DAYS_BETWEEN`/`CURRENT_DATE`）示範，一次啟用成功、76 筆驗證正確 | 對照 REST rs05 的 WHERE 條件組合手法；呼應 `.claude/rules/sap-adt-mcp.md` 第 10 節 Open SQL JOIN 限制 | 已驗收 |
