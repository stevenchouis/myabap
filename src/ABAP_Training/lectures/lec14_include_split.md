# 講義 14：INCLUDE 拆檔——TOP / F01 慣例（授課順序：接在講義 12 之後、講義 13 之前）
...
- 主程式只剩骨架：一眼看懂整支程式
- **拆檔不是模組化**：好處在可讀性、認得 SAP 慣例、多人同時修改
- 建立、啟用 INCLUDE 的注意事項
...
INCLUDE 常被誤認成「模組化」，其實它只是**整理原始碼檔案**，對照講義 8／15 就看得出差別：

| | FORM（講義 8） | Function Module（講義 15） | INCLUDE（本講） |
|---|---|---|---|
| 可以被「呼叫」 | ✔ `PERFORM` | ✔ `CALL FUNCTION` | ✘ 只會被原地展開 |
| 有自己的區域變數 | ✔ | ✔ | ✘ 用主程式的環境 |
| 有介面（參數） | ✔ `USING`／`CHANGING` | ✔ `IMPORTING`／`EXPORTING`… | ✘ |
| 目的 | 切出可重複呼叫的邏輯 | 全系統共用的邏輯 | 讓大程式好讀好管 |

那拆檔的好處是什麼？都是「給人看、給團隊用」的，對程式行為沒有任何影響：

1. **大程式好讀、好找**：宣告、事件流程、FORM 各放一處，主程式只剩骨架，雙擊 INCLUDE 名稱就跳過去。
2. **認得 SAP 的慣例**（最重要的一點）：既有客製報表幾乎都拆成 `_TOP`／`_F01`（如本專案 ZDQM 系列）；講義 15 的 Function Group 本身就是系統用 TOP／UXX／U01 這組 include 組成的，Module Pool（Dialog 程式）也是。不學這一講，自己寫程式不受影響，但打開公司既有程式、標準程式會看不懂結構。
3. **多人同時修改**：每個 INCLUDE 是獨立的 Repository 物件、各自鎖定，兩個人可以同時改同一支報表的不同 include。
4. **共用**：多支程式可以引用同一個 include——這是雙面刃，見上一段的 Where-Used 提醒。

所以本講安排在「程式真的變大」的時候：講義 12 的報表已經有選擇畫面、事件、JOIN、分頁，下一講的總整理（講義 13）還會更大，正是該拆的時機。
...
- 共用 INCLUDE 要格外小心：它是「隱形的耦合」，新程式間要共用邏輯，優先考慮 Function Module（講義 15，已學過）或 Class，而不是共用 INCLUDE。
...
完成 [ex14](../ex14_include_split.md)：把一支完整的報表拆成主程式＋`_TOP`＋`_F01` 三支，驗證行為與拆檔前完全一致。
