...
| | FORM | FM | INCLUDE |
|---|---|---|---|
| 可以被呼叫 | ✔ `PERFORM` | ✔ `CALL FUNCTION` | ✘ 原地展開 |
| 自己的區域變數 | ✔ | ✔ | ✘ |
| 介面（參數） | ✔ | ✔ | ✘ |
...
1. 大程式好讀好找：主程式只剩骨架
2. **認得 SAP 慣例**：客製報表 `_TOP`／`_F01`、Function Group（TOP／UXX／U01）、Module Pool
3. 每個 INCLUDE 各自鎖定 → 多人可同時修改
4. 可共用（雙面刃，先 Where-Used）
...
- 共用 INCLUDE 是「**隱形的耦合**」：
  新程式間要共用邏輯，優先 Function Module（講義 15）
