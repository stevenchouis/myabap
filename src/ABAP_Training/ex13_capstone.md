...
> 授課順序：接在練習 14（INCLUDE 拆檔）之後，是「傳統報表」階段的總整理；之後進入 ALV（ex09）與 DDIC（ex25 起），全課程最後一題是 ex23（題號因 SAP 物件已命名而未重編，見 README）。
...
2. 營收是「各幣別混在一起」的——怎麼依幣別分組小計？（提示：ex20 的 SORT + AT END OF）

> 「把明細改成 ALV 輸出」這題移到下一階段：學完 ex09 後，ex09 的延伸挑戰會直接拿本題的報表來改。
