...
> - 新增 ex28（2026-07-31，歷經四版改版為最終版）：「客製 Table Maintenance 的權限防護與並行控制」，把 ex21（Z 表＋外鍵＋SM30）跟 ex27（Lock Object）串成企業級的完整模式——SM30 原生權限只到表格層級，業務維度（如「只能改自己負責的航空公司」）要自己包一層 Authorization Wrapper。**歷經四版設計**：初版是「工廠(WERKS)＋通用參數」，使用者實測 Lock／篩選通過後認為「維護完全沒接到計算，教學意義有限」；第二版把清單程式的按鈕改用 SE93 Parameter Transaction（過程中一個 GUI 設定欄位被 Claude 猜錯、使用者截圖糾正，見 `.claude/rules/sap-adt-mcp.md` 相關記錄）；第三版整個改成「航空公司折扣」設計（表 `ZTR28_CDISC`、權限物件 `ZTR28_CARR`、Lock Object `EZTR28_CARR`，新增 `ZR_TR28_PRICE_CALC` 計算報表＋`ZR_TR28_SEED_DEMO` 種子程式）；**第四版（最終版）拿掉了 `ZR_TR28_PARAM_LIST`**——這支「清單＋沒有保護對照按鈕」程式的功能跟 `ZR_TR28_PRICE_CALC` 補齊按鈕/ALV 後完全重複，教學上多餘，予以移除（物件因 ADT 無刪除 API 而保留、原始碼改成說明性 stub），本題最終只留**兩支主程式**：`ZR_TR28_PARAM_MAINT`（維護入口）與 `ZR_TR28_PRICE_CALC`（主要報表，JOIN 標準表 `SPFLI`/`SFLIGHT` 套用折扣算最終票價，`REUSE_ALV_GRID_DISPLAY` 輸出，選取畫面按鈕可直接跳去維護）。全部程式皆已建立、啟用、語法檢查通過，折扣計算邏輯（`666.00×0.85=566.10`）已驗證正確。**教學重點**：Lock Object 鎖定範圍要對應「使用者實際爭搶的資源邊界」；T-code 要指給 Wrapper 程式、不能指給裸的 SM30；主檔維護的值不是憑空存在，會被下游報表拿去算錢。**兩個意外發現**：① `REUSE_ALV_GRID_DISPLAY` 在 `programrun` 無頭環境通常會退化成文字清單輸出、可以直接驗證，不像 OOP 課教的 `cl_salv_table` 會卡住斷線；② 但這個好處不是絕對可靠——`programrun` 對某個特定物件一旦卡住過一次，之後不管怎麼改程式碼都會持續卡住（跟程式碼無關，是該物件的無頭執行 Session 被卡死），詳見 `.claude/rules/sap-adt-mcp.md` 第 38 節。**尚待使用者建立/修改 6 項 GUI-only 物件/設定**（SU21 權限物件 `ZTR28_CARR`、PFCG 角色 `ZTR28_MAINT_ROLE`（沿用既有角色，只改授權內容）、SE11 Lock Object `EZTR28_CARR`、SM30 Table Maintenance Generator（新 Function Group `ZFG_TR28B`）、SE93 T-code `ZTR28_MAINT`〔沿用不用改〕、SE93 Parameter Transaction `ZTR28_SM30`〔只需修改 Default Values 指向新表〕）**＋兩支主程式的 Selection Texts**（`P_CARRID`／`P_CONNID`／`P_DISP`，同樣 GUI-only），建好後才能做完整端對端驗證。基礎課程至此共 28 題。
