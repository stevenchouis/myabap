...
**ex01 → ex02 → ex17（流程控制）→ ex18（字串日期）→ ex03 → ex04 → ex05 → ex19（除錯）→ ex16（Field-Symbol）→ ex06 → ex07 → ex08 → ex09 → ex24（可編輯 ALV，進階選修）→ ex10 → ex22（訊息與文字元素）→ ex11 → ex20（群組小計）→ ex12 → ex14（INCLUDE 拆檔）→ ex15（Function Module）→ ex25（Data Dictionary 總覽/Global Type）→ ex21（Z 資料表）→ ex13（期末綜合實作一）→ ex23（期末整合練習二，全課程最後一題）**
