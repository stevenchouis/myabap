...
| ale03 | IDoc 基礎結構與監控 | Control/Data/Status Record 結構拆解；監控交易 `WE02`/`WE05`；測試工具 `WE19`（複製既有 IDoc 修改重送）；跑一次標準訊息類型的最小端對端範例 | 承 ale02 | 未出題 |
| ale04 | Master Data 分送實戰——Change Pointer 機制端對端 | `BD52` 欄位層級啟用（挑一個標準訊息類型，如 Customer/Cost Center Master）；異動主資料→`RBDMIDOC` 觸發→監控結果；**在兩個 Client 各自配置 Partner Profile，練熟 Inbound/Outbound 判斷** | 承 ale03 | 未出題 |
| ale05 | Transaction Data 觸發機制 | Output Determination（`NAST`／輸出類型如 `RD04`）vs. 手動/Enhancement 觸發，跟 ale04 的框架自動化機制對照 | 承 ale04 | 未出題 |
| ale06 | 自訂 IDoc 擴充 | Extension Segment（`WE30`/`WE31`）掛到既有標準 Basic Type；或自訂完整 Z 訊息類型（`WE81`+`WE30`+`WE31`），兩種擴充路線的取捨；**待驗證：ale01 已查證 Segment Type 本質是 DDIC Structure（`TABL`），Claude 能否用 `sap_create_object`/`sap_set_source`（`objectType=STRU`）自動建出 Structure、再請使用者在 `WE31` GUI 完成「登記成 Segment」這一步** | 承 ale03 | 未出題 |
