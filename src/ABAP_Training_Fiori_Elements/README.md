...
### 延伸篇（2026-08-24，課綱總覽發現的缺口）

fe01～fe12 結案後做了一次全課程總覽，比對「ABAPER 做 RAP+Fiori 專案幾乎必然會碰到、但這門課沒教」的主題，抓出兩個最值得優先補的缺口——都是「後端物件其實已經在別的課建好，只是從沒在真正的 VS Code App 裡驗證過」的半成品案例：

| # | 主題 | 環境 | 內容重點 | 狀態 |
|---|---|---|---|---|
| fe13 | Value Help 真實驗證 | BTP Trial | 沿用 fe08 的 `fe08taskprojection`，**不新增任何 ABAP 物件**，直接在真正的 App 裡點開 rc08 已建好的 `ZI_RC08_STATUS_VH` Value Help（`status` 欄位），補上 rc08 只驗證到 Eclipse Preview、fe08 沒接著驗證的最後一步；順帶釐清「DDLX 不會透過 `as projection on` 自動繼承」這個容易誤解的細節 | ✅ 已出題並實機驗證成功（2026-08-24） |
| fe14 | 訊息模型 | BTP Trial | 全課程 grep 確認 `MessageToast` 只出現在教學示範，RAP `reported`／`%msg` 錯誤訊息在畫面上長什麼樣從未教過；`ZI_RC05_NOTE` 新增一條 `validateTitleContent` Validation（Title 不可跟 Content 相同），示範 `new_message_with_text`＋`%element` 語法（查證官方文件 `ABAPDERIVED_TYPES_MSG`）。**Part A 實機測試意外推翻原始草稿**：`field(mandatory) title` 完全不擋空白存檔，查證官方文件 `ABENBDL_FIELD_CHAR` 才發現 `mandatory`（僅 UI 提示，無執行期檢查）跟 `mandatory:create`（真的會擋，且會讓框架跳出 Create Dialog）是兩種不同機制——講義已改寫成「三種必填/驗證機制對照表」。**Part B 實測又抓到一個真正的 ABAP 語法錯誤**：`LOOP AT itab WHERE comp1 = comp2` 不支援比較同一張表的兩個欄位（`Field "CONTENT" is unknown.`），改成 `LOOP` + `IF` 工作區比較後修正成功。過程中也修正兩處把本機快照檔名誤植成 Eclipse 開啟指令的講義措辭 | ✅ 已出題並**全部端對端實機驗證成功**（2026-08-24） |
| fe04 附註 | Object Page／Custom Page 衝突 | — | fe14 實測時發現 fe04 把 `NoteObjectPage` 換成唯讀 Custom Page，導致 fe14 完全無法輸入欄位值——已在 fe04／fe14 兩份講義互相加註提醒，測 fe14 前要先把 `manifest.json` 切回標準範本 | ✅ 已修正並記錄（2026-08-24） |
| fe15 | Message Class 與多語系維護 | BTP Trial | fe14 用 `new_message_with_text` 圖方便，正式專案常需要可重複使用、可翻譯的 T100 Message Class；ABAP Cloud 沒有 `SE91`，查證確認要用 Eclipse ADT（表格式編輯器）建立、`new_message( id/number/severity/v1~v4 )` 語法引用（查證官方 RAP Development Guide 真實範例程式碼，非猜測）；多語系翻譯查證確認要用 **Maintain Translations** 這個 Fiori App（取代 `SE63`/`SOTR_EDIT`），完整記錄 Create Project→Add Text Source→下載 XLIFF→翻譯→上傳→Publish 流程；直接在系統上查證確認多個其他學員留下的真實 `MSAG` 物件佐證機制存在；嘗試用 `create_object_programmatically` 自動建立 `ZFE15_MSG` 失敗（同 fe12 已知的 deserializing error），沿用「Eclipse 建立、Claude 設計＋事後驗證」分工。**實測發現：Message Class 訊息文字存檔即生效，不需要 `Ctrl+F3` 啟用**（跟 DDLS/BDEF/CLAS 不同，呼應傳統 `SE91` 沒有 Inactive/Active 版本概念的行為）；**用「改 Message Class 文字、不碰 ABAP 程式碼」的方法實機證實 `new_message` 真的是執行期讀取**（001 訊息改成測試文字後畫面截圖確認同步變化）。**Maintain Translations App 完整流程也已實機走完**：這個共用帳號在這個 App 沒有踩到 fe11 記錄過的權限邊界，Create Project→Add Text Source→下載 XLIFF→上傳→Publish 全部成功；過程發現兩個真實畫面行為：① Text Source 的「Add」按鈕要先建立好專案才能點，建立對話框裡是反灰的；② 首次翻譯下載的 XLIFF 完全沒有 `<target>` 元素，要自己手動新增在 `</source>` 之後。**最後用 `sap-language=zh-TW` 網址參數完整驗證翻譯生效**：訊息文字正確顯示繁體中文（含 `&1` 正確代入 `note_id`），且證實這個參數連 Fiori Elements 框架本身的標準按鈕文字也會一併切換語言，不是只影響自訂的 Message Class | ✅ 已出題並**全部六項端對端實機驗證成功**（2026-08-25）——延伸篇 fe13～fe15 至此全部完整驗證，正式結案 |
| fe16 | Overview Page——跨服務儀表板 | BTP Trial | 查證確認 Overview Page（OVP）是 fe01～fe15 完全沒碰過的三個標準 Floorplan（OVP／ALP／Worklist）裡唯一值得獨立開課的一個——ALP 在 V4 世代已整併進 List Report（跟 fe03 高度重疊），Worklist 已被市場淘汰；OVP 則是**唯一能在同一個 App 內混用多個不同 OData Service**的 Floorplan（官方文件明確記載：「You can aggregate data from multiple entities into cards ... you can use both services within the same app」），根本原因是它是卡片容器架構、不像 List Report/Object Page/ALP 整頁綁死單一 Service 的 Metadata。動手練習沿用 fe01 的 `ZRC08_SB`（Note）與 fe08 的 `ZRC01_SB`（Task）兩個既有服務，Generator 產生 Overview Page 骨架後手動在 `manifest.json` 補第二個 `dataSource`／`model`，各掛一張 `sap.ovp.cards.v4.list`／`sap.ovp.cards.v4.table` 卡片，示範全域篩選列「只對同名欄位的卡片生效」的取捨 | ✅ 已出題並**端對端實機驗證成功**（2026-08-25，Part A／Part B 全部驗收：Generator 正確產生 Overview Page 骨架，手動補上 Task 服務＋卡片後，畫面正確並排顯示 Note／Task 兩張卡片＋全域篩選列；過程中修正兩個真實踩坑——manifest.json 巢狀重複貼錯位置、`@sap/generator-fiori:ovp` 範本的主 model 命名是 `mainModel` 不是匿名 `""`） |
...
### 課程總覽（2026-08-24）發現的缺口，尚未加入課程的項目

fe13～fe15 已經補上「Value Help 真實驗證」跟「訊息模型」這兩個當時列為最優先的缺口。當時一併列出、**還沒動手**的其餘項目，記錄在這裡供下次開課參考：

- **CDS Hierarchy 從未在 Fiori Elements 樹狀 UI 裡實際渲染過**——CDS 課程 cds14／cds16 已經照抄標準物件 `I_GLAccountHierarchyNode` 建好真實可用的 Hierarchy CDS View，但「樹狀展開的真實呈現效果」一直只留給使用者在 Eclipse Data Preview 驗證，從沒真的接成一個 Fiori Elements 樹狀表格畫面。Feature Showcase 有現成範例（`RAP Treeview for Hierarchy`，`@OData.hierarchy.recursiveHierarchy`＋`@UI.facet type: #LINEITEM_REFERENCE`）可以照抄
- **Semantic Object／Intent-based Navigation（跨 App 導覽）完全沒教**——企業 Fiori 落地常見情境（例如點客戶編號跳到另一個獨立 App），這門課從 fe01 到 fe15 完全沒有一堂涉及
- **附件／檔案上傳（`@Semantics.largeObject`）完全沒教**——企業應用常見需求，這門課目前的資料模型（Note／Task／Order）都沒有附件欄位，Feature Showcase 有完整範例可以照抄
- **OData V2 vs V4 差異只在 fe06 意外碰到，沒有正式整理成對照筆記**（優先度較低，補一段說明即可，不需要整堂課）
...
- [x] fe03 補充內容已實機驗證成功（2026-08-24 出題，2026-08-25 驗證；「List Report 篩選條件的預設值——`@UI.selectionVariant`／`@UI.selectionPresentationVariant`」延伸段落＋動手練習 4／5；`annotation.xml` 已寫入不帶 `Qualifier` 的 `UI.SelectionVariant`（`note_id EQ 'RC05TEST02'`），畫面截圖確認 `Notes (1)` 一開啟就正確套用篩選、篩選框自動帶入 chip，完全不用手動操作。**過程中推翻原本抄自官方文件的寫法**：SAP 官方《Configuring Default Filter Values》範例寫 `Qualifier="Default"`，實測完全無效；真正生效的寫法是**完全不寫 `Qualifier` 屬性**（對照另一份官方文件《Configuring Default Settings》的 fallback 邏輯「checks for a default (unqualified) UI.SelectionVariant」才確認）——這是這門課第一次抓到官方文件本身自相矛盾、需要靠實測才能定案的案例。**動手練習 5（優先順序規則）也已實機驗證成功**：真正在後端 `ZI_RC05_NOTE` 的 Metadata Extension（透過 `abap-remote-fs` MCP `connectionId: abap_cloud` 直接寫入既有物件並啟用，不是改本機快照檔案，也不需要使用者手動開 Eclipse）加上 `@Consumption.filter.defaultValue: 'MYID01'`，畫面篩選欄位確認仍顯示本機 `UI.SelectionVariant` 的 `RC05TEST02`，`MYID01` 被忽略，跟官方優先順序規則吻合；**過程中意外發現 `abap-remote-fs` MCP 其實能直接編輯 BTP ABAP Cloud 系統上的既有物件**（含之前以為做不到的 DDLX Metadata Extension），詳見 memory `fiori-elements-tooling`。`@UI.selectionPresentationVariant`（排序＋篩選組合）部分仍只是講義文字未驗證，留待後續）
...
- [x] fe16 出題並端對端實機驗證成功（2026-08-25，Overview Page 跨服務儀表板；查證確認 OVP／ALP／Worklist 三個課程從未涵蓋的標準 Floorplan 裡，只有 OVP 值得獨立開課——ALP 在 V4 已整併進 List Report、Worklist 已被市場淘汰；查證官方文件確認 OVP 是唯一能在同一個 App 混用多個不同 OData Service 的 Floorplan（每張卡片各自 `settings.model`），沿用 fe01 的 `ZRC08_SB`（Note）＋fe08 的 `ZRC01_SB`（Task）兩個既有服務、不新增 ABAP 物件；`fe16overviewpage` 專案已產生並實機驗證：Note／Task 兩張卡片並排顯示＋全域篩選列正常運作，過程中修正兩個真實踩坑——`manifest.json` 巢狀重複貼錯位置、`@sap/generator-fiori:ovp` 範本的主 model 命名是 `mainModel` 不是匿名 `""`）
