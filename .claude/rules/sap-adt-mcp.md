...
- **⚠️ 待驗證的推論**：quickSearch 查詢一個標準 Function Exit 所屬的 Function Group（如 `V05E`，SD 銷售訂單相關）發現，使用者實際寫程式碼的地方（`CMOD` Project 指派後產生的 `<Function Exit 名>ZZ<後綴>` Include，如 `V05EZZAG`）其實是普通的 **`PROG/I` Include**——理論上可以套用第 1 節「INCLUDE 讀寫 workaround」（`programs/includes` 路徑）直接用 `sap_get_source`/`sap_set_source` 讀寫。也就是說 **CMOD 專案本身（指派 Enhancement、Activate Project）是 GUI-only，但 Function Exit 的程式碼內容可能是 Claude 能自動讀寫的**——這點只是根據系統既有物件結構觀察到的推論，**尚未實際寫入驗證**，下次遇到 Function Exit 相關需求時要先實測確認（例如試著對一個既有 CMOD Project 底下的 ZZ Include 跑一次 `sap_get_source`）再下結論，不能直接假設成立。
