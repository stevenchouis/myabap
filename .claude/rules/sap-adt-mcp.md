...
## 52. RAP Validation 跟 Determination 同一套 obsolete 語法家族，但 `failed`/`reported` 型別在 `FOR MODIFY` vs. `FOR DETERMINATION`/`FOR VALIDATION` 之間不一致；Unmanaged 手寫驗證沒有 Managed 的「整批拒絕」保證（2026-08-16 實測，RAP 課程 rap06）

- **官方文件 `ABENBDL_VALIDATIONS` 對可用範圍的描述跟第 51 節 Determination 一字不差同一條規則**：Managed 可以用；Unmanaged **僅限 Draft-enabled**；「Caution: Not available for unmanaged, non-draft RAP BOs.」——再次確認這是 RAP 框架設計本身的限制，不是這系統特有的。
- **Validation 只能 `on save`，沒有 `on modify` 這個選項**（跟 Determination 不同，Determination 有 `on save`/`on modify` 兩種）；語法：`validation <名稱> on save { field <欄位>; }` 或 `{ create; update; delete; }` 觸發條件。
- **Handler Method 一樣要用 obsolete `FOR VALIDATION`（不是官方新式 `FOR VALIDATE ON SAVE`），套用第 51 節已經確認過的規則直接成功，沒有再逐步踩錯**：
  ```abap
  METHODS validateStatus FOR VALIDATION Task~validateStatus
    IMPORTING keys FOR Task.
  ```
- **⚠️⚠️ 新發現：`failed`/`reported` 的型別規則，`FOR DETERMINATION`/`FOR VALIDATION` 跟 `FOR MODIFY`（`create`/`update`/`delete`）不一致**：
  - 在 `FOR VALIDATION` Handler 裡（單一實體 BDEF），`APPEND ... TO failed-task.` 報 `"FAILED" is a table without a header line and therefore does not have a component called "TASK".`——`failed`/`reported` 這裡**直接是表格**，要用 `APPEND ... TO failed.`（不加 `-task`），跟第 51 節 Determination Handler 的結論一致。
  - 但在 `FOR MODIFY`（`create`）Handler 裡，反過來 `APPEND ... TO failed.`（不加後綴）報 `"FAILED" is not an internal table.`——這裡 `failed`/`reported` 是**結構**，要用 `APPEND ... TO failed-test.`（`-test` 是 BDEF 裡宣告的實體別名）。
  - **推測原因**：`FOR DETERMINATION`/`FOR VALIDATION` Handler 綁定到「某個特定 Determination/Validation 名稱」，該名稱只能屬於一個實體，衍生型別在單一實體 BDEF 下直接簡化成表格；`FOR MODIFY` Handler 綁定的是「CUD 操作」本身，即使 BDEF 只有一個實體，衍生型別依然固定用「按實體別名分類的結構」表示（因為理論上一個 Modify Handler 未來擴充時可能涉及多個實體/關聯）。
  - **實務規則**：這系統上寫 RAP Handler Method 內部程式碼操作 `failed`/`reported` 時，**先看這個方法是哪一種 Handler**（`FOR MODIFY` 要加 `-<alias>`，`FOR DETERMINATION`/`FOR VALIDATION` 不用），不能憑印象套用同一種寫法，也不能只看官方範例（官方範例的 BDEF 通常有多個實體，兩種 Handler 看起來都用 `-<alias>`，掩蓋了「單一實體時會簡化」這個細節）。
- **`%cid` vs. `%key`**：`FOR MODIFY`（`create`）情境下，被拒絕的資料還沒有正式 Key，`failed-test`/`reported-test` 用 `%cid`；`FOR VALIDATION`（`on save`，資料已存在）用 `%key`——這點延續第 51 節已經教過的「% 開頭欄位要看當下情境有沒有正式 Key」規則。
- **⚠️⚠️ Unmanaged 手寫驗證沒有 Managed 官方保證的「一筆不合法、整批全部拒絕」語意**：官方文件明講 Managed Validation 失敗時「one inconsistency leads to a rejection of all the content in the transactional buffer」——同一個 RAP 交易裡只要有一筆不合法，連同批合法資料一起被拒絕。但這門課的 Unmanaged `create` 方法用 `LOOP AT it_create` 逐列處理，不合法的那列 `CONTINUE` 跳過、合法的那列照樣 `INSERT`——**已用 `programrun` 實測驗證**：一次 EML 同時傳一筆合法（`VAL_TEST01`）＋一筆不合法（`VAL_TEST02`，`descr` 空字串），結果 `failed` 正確回報 1 筆，但 `VAL_TEST01` 依然成功寫進資料庫，跟 Managed 的「全部拒絕」語意不同。**這不是 bug，是 RAP 框架對 Unmanaged 的設計哲學**：框架不會替手寫邏輯強加任何語意保證，程式碼想要哪種行為（逐列處理 vs. 整批檢查完才決定要不要寫入）要自己實作。

## 53. RAP Action 不需要 obsolete 語法（跟 Determination/Validation 不同）；Unmanaged Action 沒有 Draft 限制，是這系列課程第一個真正能宣告式執行的 Unmanaged 機制；RESULT 衍生型別在 EML 呼叫端是直接表格、不用 `-<alias>`（2026-08-16 實測，RAP 課程 rap07）

- **官方文件 `ABENBDL_ACTION_NONFACTORY` 對可用範圍的描述跟第 51／52 節的 Determination/Validation 完全不同，沒有「Caution: Not available for unmanaged, non-draft」這條但書**：Action 明講「Managed RAP BO」「Unmanaged RAP BO」都支援，無 Draft 限制——這是 rap05～rap07 三課裡第一個 Unmanaged 有真正宣告式語法可用的機制。
- **⚠️⚠️ 重大發現：Action 的 Handler Method 完全不需要 obsolete 語法轉換，直接套用官方現行語法一次就編譯成功**：
  ```abap
  METHODS markDone FOR MODIFY
    IMPORTING keys FOR ACTION Task~markDone RESULT result.
  ```
  **原因**：官方文件明講 Action 的自訂邏輯「must be implemented in the RAP handler method `FOR MODIFY`」——Action 沒有像 Determination（`FOR DETERMINE`/`FOR DETERMINATION`）、Validation（`FOR VALIDATE`/`FOR VALIDATION`）那樣擁有自己專屬的 Handler 類別，而是掛在 `FOR MODIFY` 底下（跟 `create`/`update`/`delete` 共用），用 `FOR ACTION <alias>~<action名稱>` 子句指定綁定哪個 Action。這系統從 rap03 就已經確認 `FOR MODIFY`/`FOR CREATE` 這個類別本身官方現行語法可以直接編譯——**這系統的「新舊語法落差」精確發生在 Determination/Validation 各自專屬的 Handler 類別層級，不是全面性只認舊語法**，遇到新的 RAP BDL 元素，要先試官方現行語法，編譯失敗才回頭查 obsolete 版本，不能一概而論套用前面課程學到的規則。
- **BDEF 語法**：`action <名稱> result [1] $self;`——`result [1] $self` 是可選子句（`$self` 代表回傳型別跟實體本身相同，`[1]` 是回傳基數）；沒有回傳值的 Action 可以完全省略 `result` 子句。Action 預設是 instance-bound（綁在具體實例上），加 `static` 關鍵字才是不綁定實例的靜態操作。
- **`READ ENTITIES` 不支援官方教材偶爾出現的 `ALL FIELDS` 簡寫**：`READ ENTITIES OF ... ENTITY task ALL FIELDS WITH CORRESPONDING #(keys) RESULT DATA(tasks).` 啟用報 `"FIELDS (" expected, not "ALL FIELDS".`——這系統的 `READ ENTITIES` 一律要明確列出 `FIELDS ( field1 field2 ... )`，這條規則延續第 51 節 Determination 已經用過的寫法，這次確認 `ALL FIELDS` 這個簡寫在這系統完全不能用。
- **`RESULT` 衍生型別的存取規則，在方法本體內跟在 EML 呼叫端不一樣**：
  - Handler Method 本體內組裝 `result` 時：`result = VALUE #( FOR ls_task IN tasks ( %key = ls_task-%key %param = ls_task ) ).`——每一列有 `%key`（識別）跟 `%param`（實際回傳的實體資料），跟 `failed`/`reported` 的 `%key`+`%msg` 是同一套「`%` 開頭表識別、後面接內容」命名慣例。
  - **在 EML 呼叫端（`MODIFY ENTITIES ... EXECUTE ... RESULT DATA(ls_result)`），`ls_result` 直接是表格，不用像 `FAILED`/`REPORTED` 那樣加 `-<alias>`**（`LOOP AT ls_result` 可以直接動，`LOOP AT ls_result-test` 反而會報 `"LS_RESULT" is a table without a header line and therefore does not have a component called "TEST".`）——但同一句 EML 陳述式裡的 `FAILED`/`REPORTED` 依然要用 `-<alias>`（`ls_failed_action-test`）。**同一句 EML 陳述式的三個回應參數（`RESULT`/`FAILED`/`REPORTED`）型別規則不統一，要逐一確認，不能假設三個都一樣**。
  - 讀取 `RESULT` 表格的內容一樣要透過 `%param`：`LOOP AT ls_result INTO DATA(ls_res). ... ls_res-%param-descr ...`（直接 `ls_res-descr` 會報 `does not have a component called "DESCR"`）。
- **EML `EXECUTE` 語法**：`MODIFY ENTITIES OF <view> ENTITY <alias> EXECUTE <action名稱> FROM VALUE #( ( %key-<key欄位> = ... ) ) RESULT ... FAILED ... REPORTED ....`——跟 `CREATE`/`UPDATE` 用 `FIELDS ( ... ) WITH VALUE #( ... )` 不同，`EXECUTE` 用 `FROM VALUE #( ... )`，每一列只要給 `%key-` 開頭的識別欄位（不需要 `FIELDS` 子句，因為通常不需要額外傳欄位資料）。
- **Unmanaged Action 方法本體的 `UPDATE ... SET` 語法要注意兩點**（新式 Open SQL、用 `@` escape 宿主變數的情境）：① `SET` 子句多個欄位之間要用逗號分隔（`SET a = @x, b = @y`，不加逗號報 `The elements in the "SET" list must be separated using commas.`）；② `WHERE` 子句不能明寫 Client 欄位（`WHERE client = @sy-mandt AND ...` 報 `The client field "CLIENT" cannot be specified in the WHERE condition. Client handling is performed by the compiler.`，跟第 10 節記載的 JOIN ON 條件不能寫 MANDT 是同一條規則的 UPDATE 版本）。
- **已用 `programrun` 完整驗證成功，且是真正的宣告式 Action（不是像 rap05/rap06 那種因為框架不支援才手寫的等效版本）**：`ZI_RAP03_UMTEST` 加 `action touch result [1] $self;`，`ZBP_I_RAP03_UM4` 實作直接重用 rap05 已經寫好的 `determine_creation_info( )` 私有方法（同一段邏輯，這次是被 Action 呼叫，rap05 是被 `create` 呼叫，體現「同一段邏輯可以被系統裡任何需要它的地方重用」）。驗證程式建立一筆資料、記錄 `created_at`、等 1 秒後 `EXECUTE touch`，確認 `created_at` 真的變新（從 `...135113...` 變成 `...135114...`，差約 1 秒，吻合程式裡的 `WAIT UP TO 1 SECONDS`），`RESULT` 正確回傳改完的資料。

## 54. RAP Composition／Create-by-Association：子實體 `lock dependent` 語法、Handler `FOR CREATE <alias>\_<assoc>`，以及最關鍵的「父 Key 是自訂值時要用 `%key` 不是 `%cid_ref`」靜默失敗坑（2026-08-16 實測，RAP 課程 rap08，暫時性驗證物件）

- **背景**：rap08 是「教學分工原則」第一次真正落地的課程（Table/CDS View/BDEF 主要物件改由使用者在 Eclipse 建立，Claude 事後驗證），但為了不把未驗證過的語法教給使用者，Claude 先用**暫時性驗證物件**（`ZRAP08V_H`/`ZRAP08V_I` 表格、`ZI_RAP08V_H`/`ZI_RAP08V_I` Managed CDS+BDEF、`ZI_RAP08VU_H`/`ZI_RAP08VU_I`/`ZBP_I_RAP08VU_H` Unmanaged CDS+BDEF+Class、`ZR_RAP08V_DEMO` 驗證程式，這些跟課程正式命名`ZRAP08_ORDER`系列不同，用意是「R&D 用，不算課程正式物件」）把語法在系統上完整跑過一輪，這是本課程第一次「Claude 建立驗證物件但不建立正式課程物件」的模式。
- **Composition／Association 宣告語法本身完全沒有問題，Managed／Unmanaged 通用**：`composition [0..*] of <子CDS View> as _<別名>`（父）、`association to parent <父CDS View> as _<別名> on $projection.<欄位> = _<別名>.<欄位>`（子）——直接讀這系統既有標準物件 `C_SalesOrderManage`/`C_SalesOrderItemManage` 的 DDL 得到的 ground truth（這兩個物件正是用這系統的舊式 `define root view`/`define view`，不帶 `entity`，composition/association 語法完全通用），不是猜測。
- **⚠️ 子實體 BDEF 的 `lock` 子句，這系統要用 `lock dependent ( <子鍵> = <父鍵> )`，不是官方教材／`ABENBDL_LOCKING` 文件記載的 `lock dependent by _<別名>`**：用 `lock dependent by _Header` 啟用報 `"(" expected, not "by".`，改成 `lock dependent ( order_id = order_id )` 才編譯成功——延續本課程一貫「這系統文法停在較舊版本」的模式（但注意這系統既有標準物件 `C_SalesOrderManage` 的 Unmanaged BDEF 用的正是 `lock dependent ( SalesOrder = SalesOrder )` 這種寫法，這次是直接照抄標準物件語法就對了，不需要像 Determination/Validation 那樣逐步試錯）。
- **⚠️ 子實體欄位如果是「使用者要輸入的資料」（例如子實體自己的 Key），不能標 `field(readonly)`**：一開始把 `order_id`（外鍵）跟 `item_id`（子實體自己的 Key）都標 readonly，Create-by-Association 建立時報 `The field "ITEM_ID" of entity "..." cannot be modified.`——拿掉 `item_id` 的 readonly 後解決。**判斷原則**：readonly 只該用在「框架或程式邏輯決定值、使用者不該手動指定」的欄位（`order_id` 是從父實體帶過來的外鍵、`created_at`/`created_by` 是邏輯自動填），子實體需要使用者親自指定的欄位不能是 readonly。
- **Create-by-Association 的 Handler Method 用新的衍生型別 `FOR CREATE <alias>\_<association>`**（反斜線跳脫底線開頭的關聯別名，跟 EML `CREATE BY \_<association>` 用的反斜線是同一個 ABAP 語法規則），這個宣告本身编译無誤，跟 rap07 已確認的「`FOR MODIFY` 家族用官方現行語法即可，不需要 obsolete 轉換」規則一致：
  ```abap
  METHODS create_item FOR MODIFY
    IMPORTING it_create FOR CREATE Header\_Item.
  ```
  方法本體是雙層結構：`LOOP AT it_create` 外層走過所有父實例（`ls_create-%key-<父鍵欄位>` 取得父 Key），`LOOP AT ls_create-%target` 內層走過這個父實例底下所有要建立的子實例資料。
- **⚠️⚠️⚠️ 最關鍵、最容易誤判的坑：EML 呼叫端連結父子關聯，父實體 Key 是使用者自訂值時要用 `%key-<欄位>`，用官方範例常見的 `%cid_ref` 會完全靜默失敗（無任何錯誤訊息，`sy-subrc = 0`，`FAILED`/`REPORTED`/`MAPPED` 全部是空的，但子實體就是沒有被建立）**：
  - 官方文件（`ABENBDL_DETERMINATION_ABEXA` 的 CBA 範例）用 `%cid_ref = '1'` 反查父實體（因為那個範例的父 Key `SoKey` 是 `numbering:managed` 框架自動編號，建立當下還不知道真正 Key，只能先用暫時代碼 `%cid` 建父、`%cid_ref` 反查）。這系統的驗證範例父 Key（`order_id`）是**使用者自己指定的值**，一開始就知道真正 Key，不需要 `%cid_ref` 這層反查機制。
  - 照抄官方 `%cid_ref` 寫法，**語法完全編譯通過、EML 呼叫完全沒有報錯**，但用「哨兵值」排錯技巧（在 Handler Method 裡先無條件寫入一筆固定資料，確認方法本身有沒有被呼叫到）證實：Handler Method **有**被呼叫，但傳進來的 `it_create` 是空表——問題不在 Handler 邏輯，是 EML 呼叫端「用 `%cid_ref` 反查父實體」這一步在「父 Key 是使用者自訂值」這種情境下沒有成功連結，框架安靜地放棄了整個 CBA 請求，不當作錯誤處理。
...
## 60. IDoc／ALE 字典表實測對照與 Segment 兩層結構（2026-09-21 實測，ALE 課程 ale05/ale06 出題）

- **本次 session 起 `sap-adt`／`sap-adt-home` 連線恢復，`datapreview/freestyle`（bridge `127.0.0.1:8410`）可查**；`SELECT COUNT(*) ... GROUP BY` 這類寫法要加別名（否則報 `all expressions in the projection list must have an alias name`），`NOT IN ( subquery )` 後面不能接 `ORDER BY`。
- **Segment 是兩層 DDIC Structure**：`E1MARAM`（Segment Type，欄位全是 `abap.char(n)`）與 `E2MARAM`（Segment Definition，欄位引用 Data Element，如 `matnr : matnr;`），兩者 TADIR 都是 `TABL`（套件 `IDOCLOGISTICS`）；版本化的 `E2MARAM001`…`E2MARAM008` 不是獨立 TADIR 物件，只出現在 `EDISDEF`。**Segment 不能靠 ADT 自建 Structure 註冊**：`EDISDEF`／`EDSAPPL` 只由 `WE31` 寫入，且事先建同名物件會讓 `WE31` 撞名。
- **已驗證的表與欄位**：`EDISDEF`（`SEGTYP`/`VERSION`/`SEGDEF`/`RELEASED`/`CLOSED`/`FIELDNUM`/`EXPLENG`/`APPLREL`，注意**沒有** `DEFLENGTH`）；`EDSAPPL`（`SEGTYP`/`POS`/`FIELDNAME`/`ROLLNAME`/`EXPLENG`/`ISOCODE`，即 WE31 欄位清單）；`EDIMSG`（`MESTYP`/`IDOCTYP`/`CIMTYP`/`RELEASED`/`ACTFLAG`，即 WE82 指派）；`EDBAS`（`IDOCTYP`/`PRESP`/`PWORK`/`PLAST`/`CLOSED`/`RELEASED`/`PRETYP`/`GENERATED`；**`PRETYP` 是前一版本（`MATMAS05` 的 `PRETYP`＝`MATMAS04`），不是 Extension 連結**）；`EDID4`（`DOCNUM`/`COUNTER`/`SEGNUM`/`SEGNAM`/`PSGNUM`/`HLEVEL`/`SDATA`）；`BDCP`（`CPIDENT`/`TABNAME`/`TABKEY`/`FLDNAME`/`CRETIME`/`ACTTIME`/`USRNAME`/`CDOBJCL`/`CDOBJID`）；`NAST` 有 `KAPPL`/`KSCHL`/`VSTAT`/`NACHA`，`TNAPR` **沒有** `PROGRAMM` 欄位。
- **標準 Segment 的物料號碼欄位 Data Element 是 `MATNR18`（18 碼）**，不是 40 碼的 `MATNR`（`EDSAPPL` 實測 `E1MARAM-MATNR`→`MATNR18`）；`EKKO-LIFNR` 的 Data Element 是 `ELIFN`。
- 系統上目前沒有任何客戶 Segment（`EDISDEF` 無 `Z%`）與 Extension（`TADIR` 無 `IEXT`），ALE 課程是從零開始。
- **代碼與欄位名更正（2026-09-21 實測）**：`EDIDC-DIRECT` 是 `1`＝Outbound、`2`＝Inbound（`DD07T` Domain `EDI_DIRECT`）；`EDIDC` 的 Basic Type 欄位是 `IDOCTP`（`EDP13`/`EDIMSG` 才叫 `IDOCTYP`）；`EDIDS` 的計數欄位是 `COUNTR`（從 `0` 起算，`EDID4` 才叫 `COUNTER`）；`EDIDS` 狀態文字表 `TEDS2` 的欄位是 `DESCRP`；`EDID4-SDATA` 是 LRAW，不能直接 `SELECT SDATA`（報「Only the prefixed length field can be used」）；SD 輸出的 Application 編號 `V1`＝Sales、`V2`＝Shipping、`V3`＝Billing（`NAST` 實測 `RD04` 的 `KAPPL`＝`V3`），`NACHA`＝`6` 為 EDI/ALE 媒介。
- **標準 IDoc 相關 FM／Exit 實測位置**：`MASTER_IDOC_DISTRIBUTE`（Function Group `BD11`）；Inbound FM 標準介面見 `IDOC_INPUT_MATMAS01`（`MV02`）／`IDOC_INPUT_CLFMAS`（`CLA1`），`idoc_status LIKE bdidocstat`（欄位 `docnum`/`status`/`msgty`/`msgid`/`msgno`/`msgv1..4`/`repid`）；`MATMAS` Outbound Exit＝SMOD `MGV00001` 的 `EXIT_SAPLMV01_002`（Include `ZXMGVU03`，每個 Segment 呼叫一次，`idoc_cimtype` 為 EXPORTING），`EXIT_SAPLMV02_002` 是 Inbound 用；SD 發票 Outbound Exit＝`LVEDF001` 的 `EXIT_SAPLVEDF_001`～`004`（Include `ZXEDFU01`～`04`）；MIRO BAdI `INVOICE_UPDATE`（Multi Use，`ENHS/XS`＋`SXSD/XD` 皆有，方法 `CHANGE_AT_SAVE`/`CHANGE_BEFORE_UPDATE`/`CHANGE_IN_UPDATE`）；`BAPI_ACC_DOCUMENT_POST`（`ACID_PI`）與 `BAPI_ACC_DOCUMENT_CHECK`（`ACC9`）；`EDIFCT` 為 `WE57` 對應表。
- 這個系統已有真實 `INVOIC` 歷史（2020-08～2023-04，115 筆，Partner `A1000`/`KU` Outbound、`A2000`/`LI` Inbound，Process Code `INVF`/`INVL`），ale10 直接用它當教材。

## 61. Open SQL 的 `INTO` 放在整句最後（`ORDER BY` 之後）＝新式語法標誌；基礎課「lec26 之前一律傳統寫法」的驗證方法（2026-09-24 實測，基礎課 ex06～ex28 改回傳統寫法）
